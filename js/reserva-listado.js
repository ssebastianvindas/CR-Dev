...
const llenarTabla = () => {
    ...
    reservas.forEach((reservaTemporal) => {
        ...
        btnEditar.addEventListener("click", () => {
            Swal.fire({
                title: "Está seguro que desea editar la reserva?",
                text: "La acción no se puede revertir!",
                icon: "warning",
                showCancelButton: true,
                confirmButtonColor: "#3085d6",
                cancelButtonColor: "#d33",
                confirmButtonText: "Sí, vamos a editar!",
            }).then((result) => {
                if (result.isConfirmed) {

                    let id = reservaTemporal._id;
                    localStorage.setItem('idStorageReserva', id);
                    window.location.href = '../reserva-editar.html';
